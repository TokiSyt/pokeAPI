...
def import_pokemon_ability_from_api(ability_name_or_id: str) -> PokemonAbility | None:
    ...
    names_list = [entry for entry in data.get("names", []) if entry.get("language", {}).get("name") == "en"]
    effect_entries_list = [entry for entry in data.get("effect_entries", []) if entry.get("language", {}).get("name") == "en"]
