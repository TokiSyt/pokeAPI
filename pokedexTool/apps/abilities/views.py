    ...
    def get(self, request, poke_ability_name_or_id):
        ...
        try:
            if poke_ability_name_or_id.isdigit():
                ability_obj = PokemonAbility.objects.get(Q(ability_id=poke_ability_name_or_id))

            elif not poke_ability_name_or_id.isdigit():
                ability_obj = PokemonAbility.objects.get(Q(name=poke_ability_name_or_id))
        ...
        if ability_obj_needs_update or ability_obj is None:
            ability_obj = import_pokemon_ability_from_api(poke_ability_name_or_id, self.request.user)
